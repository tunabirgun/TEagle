...
![Version](https://img.shields.io/badge/version-2.5.0-0E9E86) ![Platform](https://img.shields.io/badge/platform-Windows%20x64-1FB89C) ![License](https://img.shields.io/badge/license-Proprietary-C6432E) ![Built with](https://img.shields.io/badge/built%20with-PySide6%20%C2%B7%20Primer3%20%C2%B7%20HMMER-2B3740)
</div>
...
- **Whole-genome off-target scan** (optional, WSL) — right-click a designed primer pair to scan an organism's RefSeq genome for off-target priming sites with UCSC isPcr, entirely on your machine. The genome is downloaded once through NCBI Datasets and cached locally (a genome manager lists, downloads, and deletes cached genomes), so the search runs with **no remote query and no server-side queue** — validated end-to-end on the yeast, *Drosophila*, and human genomes. Candidate off-target products render as a to-scale gel + coordinate table, and the run is **sealed reproducibly** by assembly accession, source-genome checksum, and isPcr version.
...
**Whole-genome off-target scan** (optional, WSL) — pick an organism, then right-click a designed pair and choose *Scan whole genome for off-targets*. The organism's RefSeq genome downloads once and is cached locally (managed from *⚙ Manage genomes*); the scan then runs on your machine with UCSC isPcr — no remote query, no server-side queue — and reports candidate off-target priming sites as a to-scale gel and coordinate table. Every product is an off-target candidate (a transposable-element pair has no single designed locus), so the result is advisory; the run is sealed by the assembly accession, source-genome checksum, and isPcr version for reproducibility.

![Whole-genome off-target scan: a designed pair searched against a downloaded RefSeq genome with isPcr, rendered as a to-scale gel of candidate off-target products](docs/img/genome_scan.png)
