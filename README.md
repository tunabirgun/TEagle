...
**Whole-genome off-target scan** (optional, WSL) — pick an organism, then right-click a designed pair and choose *Scan whole genome for off-targets*. The organism's RefSeq genome downloads once and is cached locally (managed from *⚙ Manage genomes*); the scan then runs on your machine with UCSC isPcr — no remote query, no timeout — and reports candidate off-target priming sites as a to-scale gel and coordinate table. Every product is an off-target candidate (a transposable-element pair has no single designed locus), so the result is advisory; the run is sealed by the assembly accession, source-genome checksum, and isPcr version for reproducibility.
